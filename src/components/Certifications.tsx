          ...
          {sortedAndFilteredList.map((cert, index) => {
            ...
            return (
              <motion.div
                ...
                className={`group/card frosted-glass-card bento-item rounded-3xl border p-5 sm:p-6 flex flex-col justify-between shadow-2xs hover:shadow-md transition-all duration-300 relative overflow-hidden ${
                  isSpecialization
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between px-1 py-0.5 text-xs border-b border-slate-100 dark:border-white/10 pb-2.5">
                    ...
                    {cert.credentialUrl && !isCiscoCertificate(cert) && !isCapacitiCertificate(cert) ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 text-slate-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 font-semibold text-xs transition-colors hover:underline cursor-pointer"
                  ...
                  {/* Certificate Title */}
                  <h3 className="text-base sm:text-lg font-bold text-slate-950 dark:text-white tracking-tight leading-snug group-hover/card:text-blue-600 dark:group-hover/card:text-blue-400 transition-colors">
                    {cert.name}
                  </h3>

                  {/* Curriculum Focus */}
                  {cert.focus && (
                    <p className="text-xs text-slate-700 dark:text-slate-200 leading-relaxed bg-slate-100/90 dark:bg-slate-800/90 p-2.5 rounded-xl border border-slate-200/80 dark:border-white/10">
                      <strong className="text-slate-900 dark:text-white font-semibold">Focus:</strong> {cert.focus}
                    </p>
                  )}
                  ...
                  {cert.skillsVerified && cert.skillsVerified.length > 0 && (
                    <div>
                      <span className="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5">
                        Key Technical Skills Verified
                      </span>
                      <div className="flex flex-wrap gap-1.5">
                        {cert.skillsVerified.map((skill, idx) => (
                          <span
                            key={idx}
                            className="px-2 py-0.5 rounded-lg text-[10px] sm:text-[11px] font-mono bg-slate-100/90 dark:bg-slate-800/90 border border-slate-200/90 dark:border-white/10 text-slate-800 dark:text-slate-200 shadow-2xs font-medium"
                          >
                            {skill}
                          </span>
                        ))}
